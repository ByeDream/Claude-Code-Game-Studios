...
敌军系统管理 Run 中所有对手的生成、配置和难度曲线。除 Boss 战外，所有战斗均为 5v5。敌方阵容由正式武将（复用 Hero 数据模型，战胜后可劝降）和无名武将（军团长、都尉等战斗填充单位）混合组成。前期战斗中无名武将占比高以降低难度，后期逐渐被正式武将替代。每个历史剧本章节末有 Boss 战，Boss 在武将模型基础上拥有额外机制。

## Player Fantasy

**「前方来犯的敌将，可能是你未来的麾下。」**

普通战斗中的敌人不是无面杂兵——正式武将是有名有姓的历史人物，击败后你可以劝降收为己用。Boss 则是历史中的重要人物（董卓、吕布、曹操等），需要针对性策略。你不是在打怪，你是在和三国群雄交锋。
...
| 类型 | 数据模型 | 劝降 | 用途 |
|------|---------|------|------|
| **无名武将** | 简化 Hero 模型（固定模板，无独特技能） | ❌ 不可劝降 | 前期战斗填充、难度调节、Boss 召唤物 |
| **正式武将** | 完全复用 Hero 数据模型 (C-A tier) | ✅ 可劝降 | 中后期战斗主力敌人 |
| **Boss** | Hero 数据模型 + BossExtension | ❌ 不可劝降 | 章节末 Boss 战 |
...
Boss 不可劝降，但击败后有丰厚奖励（名器/稀有武将选择权）。
...
interface EnemyEncounter {
  ...
  lootTable: LootEntry[];
  persuadableHeroes: string[];        // 可劝降的正式武将 ID 列表
}
```
...
| **Bond System** | Bond → Enemy | 敌方正式武将享受阵营羁绊加成 |
...
|----------|----------|-----------|
| 劝降的武将和玩家已有武将重复 | 允许劝降，替补席满则选择替换/放弃 | 与 Hero System 一致 |
...
| 敌方正式武将的羁绊 | 敌方享受阵营羁绊加成 | 公平对称 |
| 无名武将被劝降 | 不可——无名武将没有历史身份 | 只有正式武将可劝降 |
